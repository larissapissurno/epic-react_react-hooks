...
When we pass a function instead of a value on `useState`, this is only called once when the component renders the first time,
this is called 'lazy initialization' and it's used to avoid performance bottlenecks when having heavy computation within `useState` hook.
